...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = RecursiveUnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'stylegan2':
        ...
        encoder = DownsampleUnet(input_nc, ngf)
        decoder = UpsampleUnet(input_nc, ngf, num_classes=opt.num_classes)
        net = UnetSegment(encoder, decoder)
    else:
...
def define_S(input_nc, output_nc, ngf, netS, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netS == "con_unet":
        ...
        encoder = ResnetEncoder(input_nc, opt.num_classes)
        decoder = ResnetDecoder(input_nc, opt.num_classes)
        net = ResNetSegment(encoder, decoder)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netS)
...
def define_F(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    elif netF == 'label_sample':
        net = PatchLabelSampleF(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
...
class PatchLabelSampleF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchLabelSampleF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, labels, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            B, H, W = feat.shape[0], feat.shape[2], feat.shape[3]
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)
            label = F.interpolate(labels, size=(H, W), mode='bilinear')
            label_reshape = label.permute(0, 2, 3, 1).flatten(1, 2)
            if num_patches > 0:
                if patch_ids is not None:
                    patch_id = patch_ids[feat_id]
                else:
        ...
        return return_feats, return_ids, return_labels
...
class UnetGenerator(nn.Module):
    def __init__(self, in_channels=1, output_channel=1, initial_filter_size=64, kernel_size=3, do_instancenorm=True):
        super().__init__()
        ...
        self.center = nn.Sequential(
            nn.Conv2d(initial_filter_size * 2 ** 2, initial_filter_size * 2 ** 3, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(initial_filter_size * 2 ** 3, initial_filter_size * 2 ** 3, 3, padding=1),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(initial_filter_size * 2 ** 3, initial_filter_size * 2 ** 2, 2, stride=2),
            nn.ReLU(inplace=True),
        )
        ...
        self.final = nn.Conv2d(initial_filter_size, output_channel,
                               kernel_size=1)  # kernel size for final layer = 1, see paper
    ...
    def forward(self, x, layers=[], encode_only=False):
        ...
        crop = self.center_crop(contr_3, center.size()[2], center.size()[3])
        concat = torch.cat([center, crop * concat_weight], 1)

        expand3 = self.expand_3_2(self.expand_3_1(concat))
        upscale = self.upscale3(expand3)

        crop = self.center_crop(contr_2, upscale.size()[2], upscale.size()[3])
        concat = torch.cat([upscale, crop * concat_weight], 1)

        expand2 = self.expand_2_2(self.expand_2_1(concat))
        upscale = self.upscale2(expand2)
        ...
        if len(layers) > 0 and encode_only:
            feats = []
            feats.append(center)
            feats.append(expand3)
            feats.append(expand2)
            return feats
...
class ResNetSegment(nn.Module):
    def __init__(self, encoder=None, decoder=None, mode="mlp"):
        super().__init__()
        ...
        if mode == 'mlp':
            ...
            self.head = nn.Linear(self.initial_filter_size*2**3, 128)

        assert encoder is not None and decoder is not None, \
            'model is not assigned with proper structure'

    def forward(self, x, encode_only=False):
        if not encode_only:
            center = self.encoder(x) # channel_num: ngf*2**n_downsampling
            output = self.decoder(center)

            return output
        else:
            center = self.encoder(x)
            center = self.gap(center)
            ft = self.head(torch.flatten(center, start_dim=1))
            return ft
...
class UnetSegment(nn.Module):
    def __init__(self, encoder=None, decoder=None, mode="mlp"):
        ...
        self.encoder = encoder
        self.decoder = decoder
        self.initial_filter_size = encoder.initial_filter_size

        self.gap = nn.AdaptiveAvgPool2d(1)
        if mode == 'mlp':
            self.head = nn.Sequential(nn.Linear(self.initial_filter_size*2**3, 256),
                                        nn.Sigmoid(),
                                        nn.Linear(256, 128))
        else:
            self.head = nn.Linear(self.initial_filter_size*2**3, 128)
    ...
    def forward(self, x, encode_only=False):
        if not encode_only:
            contr_1, contr_2, contr_3, contr_4, center = self.encoder(x)

            output = self.decoder(contr_1, contr_2, contr_3, contr_4, center)

            return output
        else:
            _, _, _, _, center = self.encoder(x)
            center = self.gap(center)
            ft = self.head(torch.flatten(center, start_dim=1))
            return ft
...
class DownsampleUnet(nn.Module):
    ...
    def forward(self, x, enable_concat=True):
        ...
        return contr_1, contr_2, contr_3, contr_4, center


class UpsampleUnet(nn.Module):
    def __init__(self, in_channels=1, initial_filter_size=64, num_classes=8):
        # only for segmentation not for generating fake images
        ...
        # Output layer for segmentation
        self.final = nn.Conv2d(initial_filter_size, num_classes,
                               kernel_size=1)  # kernel size for final layer = 1, see paper
    ...
    def forward(self, contr_1, contr_2, contr_3, contr_4, center):
        concat_weight = 1
