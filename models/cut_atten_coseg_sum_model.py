...
class CUTAttenCosegSumModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            for pcl_layer in self.pcl_layers:
                self.criterionPCL.append(AttenPatchNCELoss(opt).to(self.device))
    ...
    def forward(self):
        ...
        self.real = torch.cat((self.real_A, self.real_B), dim=0) if self.opt.pcl_idt and self.opt.isTrain else self.real_A
    ...
    def calculate_PCL_loss(self, src, tgt):
        ...
        total_pcl_loss = 0.0
        for f_q, f_k, label, crit, pcl_layer in zip(feat_q_pool, feat_k_pool, label_k_pool, self.criterionPCL, self.pcl_layers):
            label = torch.round(label)
            loss = crit(f_q, f_k, label) * self.opt.lambda_PCL
